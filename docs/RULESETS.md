...
default branch. CI validates this on every run via `tools/build/ruleset/check.js`.
The required rules are defined in `tools/build/ruleset/expected.json`.
...
`tools/build/ruleset/ruleset.json` from this repo. CI validates them continuously
thereafter.
