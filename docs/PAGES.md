...
**Handoff and save.** The connector and the embedded app exchange equivalent-origin messages: the connector hands the app the file's bytes (or a create instruction, for a new one), and on save the app hands the edited bytes back for the connector to write to Drive. Equivalent-origin means strict same-origin when the pages are served, widening only when they are opened from disk, where every `file://` document gets its own opaque origin and neither side can name the other; that widening is confined to the disk case because on the web a foreign site could otherwise embed the app and collect the database on save, while on disk anyone positioned to exploit it could replace the page outright. For a database that started as an open, that write is an update to the file already picked; for one that started as a create, the first save instead creates the file — still under the `drive.file` scope, since a file this app just created is exactly what that scope covers — and the connector remembers its id so every later save in the same session updates that same file rather than creating another. New files land in the root of My Drive; there's no folder picker yet. The local-download option is hidden while embedded either way. Opened on its own, `0x67.html` receives no such messages and behaves exactly as it does standalone.
