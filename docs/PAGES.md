...
Cloud storage is open to every visitor: there is no sponsorship gate, and opening a cloud vault requires only the provider's own sign-in and the master password — never a GitHub login. Building and maintaining these connectors, along with the project's security audits, is funded voluntarily through [GitHub Sponsors][sponsors]. The app invites sponsorship but never requires it, and the appeal is an unobtrusive link — for example on `index.html` — never a step in the unlock flow.
